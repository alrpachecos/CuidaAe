...
interface AuthState {
  ...
  register: (name: string, email: string, password: string, role: 'patient' | 'professional') => Promise<void>;
  logout: () => Promise<void>;
}
  ...
  initialize: async () => {
    try {
      ...
      set({
        user: null,
        isLoading: false,
        error: error instanceof Error ? error.message : 'Erro ao inicializar a aplicação'
      });
    }
  },
